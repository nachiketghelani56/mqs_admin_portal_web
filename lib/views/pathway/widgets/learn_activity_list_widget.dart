...
Widget learnActivityListWidget({required PathwayController pathwayController}) {
  return Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      if (pathwayController.mqsLearnActivity.isNotEmpty) ...[
        ...
        for (int i = 0; i < pathwayController.mqsLearnActivity.length; i++)
          Container(
            height: SizeConfig.size55,
            padding: const EdgeInsets.symmetric(horizontal: SizeConfig.size14),
            decoration: i == pathwayController.mqsLearnActivity.length - 1
            ...
            child: Row(
              children: [
                Expanded(
                  flex: SizeConfig.size3.toInt(),
                  child: Text(
                    pathwayController.mqsLearnActivity[i].id,
                ...
                Expanded(
                  flex: SizeConfig.size3.toInt(),
                  child: Text(
                    pathwayController.mqsLearnActivity[i].mqsActivityTitle,
                    style: FontTextStyleConfig.tableContentTextStyle,
                    overflow: TextOverflow.ellipsis,
                  ),
                ),
                Expanded(
                  child: PopupMenuButton<int>(
                    ...
                    onSelected: (value) {
                      if (value == 0) {
                        pathwayController.setLearnActivtyForm(index: i);
                      } else if (value == 1) {
                        pathwayController.removeLearnActivity(index: i);
                      }
                    },
                    itemBuilder: (context) {
                      return [
